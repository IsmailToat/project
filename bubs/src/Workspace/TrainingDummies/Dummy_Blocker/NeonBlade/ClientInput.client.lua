...
local function SetParryEffects(Enabled)
	local Spark = Handle:FindFirstChild("ParrySpark")
	if Spark then Spark.Enabled = Enabled end
end
...
UserInputService.InputBegan:Connect(function(Input, P)
	...
	if Input.UserInputType == Enum.UserInputType.MouseButton2 and not Debounce then
		...
		SetParryEffects(true)
		task.delay(0.25, function() if IsBlocking then SetParryEffects(false) end end)

		local Remote = Tool:FindFirstChild("CombatEvent")
		if Remote then Remote:FireServer("BlockStart") end
	end
end)
...
--// DEBUG HITBOX VISUALIZER (NEW)
-- This detects ANY hitbox (dummy or player) and shows it if setting is ON
workspace.ChildAdded:Connect(function(Child)
	if Child.Name == "HitboxVisual" then
		local Char = Player.Character
		if Char and Char:GetAttribute("DebugHitboxes") == true then
			Child.Transparency = 0.3 -- Make Visible
		end
	end
end)